...
public class Keyboard {
	...
	private static HashMap<String, ArrayList<Key>> keysLang = new HashMap<String, ArrayList<Key>>();
	...
	protected static void init(String keyboadType) {// You can change the keyboard lang in the Display class
		addAllCharsUS();
		addAllCharsDE();

		keys.addAll(getTheKeyboard(keyboadType));
		keysLang.clear();
	...
	private static ArrayList<Key> getTheKeyboard(String lang) {
		try {
			return keysLang.get(lang);
		} catch (Exception e) {
			return keysLang.get("us");
		}
	}
	...
	private static void addAllCharsUS() {
		...
		keys.add(new Key(GLFW.GLFW_KEY_MINUS, '-', '_'));
		keys.add(new Key(GLFW.GLFW_KEY_SLASH, '/', '?'));

		keys.add(new Key(GLFW.GLFW_KEY_LEFT_BRACKET, '[', '{'));
		keys.add(new Key(GLFW.GLFW_KEY_RIGHT_BRACKET, ']', '}'));

		keysLang.put("us", keys);
		// todo: add more keys
	}

	private static void addAllCharsDE() {
		ArrayList<Key> keys = new ArrayList<Key>();
		keys.add(new Key(GLFW.GLFW_KEY_Q, 'q', 'Q', '@'));
		keys.add(new Key(GLFW.GLFW_KEY_W, 'w', 'W'));
		keys.add(new Key(GLFW.GLFW_KEY_E, 'e', 'E', '�'));
		...
		keys.add(new Key(GLFW.GLFW_KEY_COMMA, ',', ';'));
		keys.add(new Key(GLFW.GLFW_KEY_PERIOD, '.', ':'));
		keys.add(new Key(GLFW.GLFW_KEY_MINUS, '�', '?'));
		keys.add(new Key(GLFW.GLFW_KEY_SLASH, '-', '_'));

		keysLang.put("de", keys);
